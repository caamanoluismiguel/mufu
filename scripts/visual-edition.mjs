export function visualEdition($,page,icon){
  $('link[href="assets/edition.css"],script[src="assets/edition.js"],#museum-index,#image-viewer,.index-toggle,.reading-progress,#home-visuals').remove();
  $('head').append('<link rel="stylesheet" href="assets/edition.css">');
  $('body').addClass('mufu-edition').attr('data-surface',page==='index.html'?'home':page.startsWith('ficha-')?'record':'archive');
  if(page==='infografia.html')return;
  const links=[['index.html','El trimestre','01'],['atlas.html','Atlas de conexiones','02'],['coleccion.html','53 objetos','03'],['infografias.html','Infografías','04'],['archivo.html','Archivo y fuentes','05'],['index.html#visita','Visita','06']];
  ...
  const header=$('.site-header,.bar,.top__in').first();
  header.append(toggle);
  if(!header.length)$('body').prepend(`<div class="edition-utility">${toggle}</div>`);
